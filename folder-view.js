...
function openFolderViewInNewWindow() {
    ...
    // ★★★ [핵심 수정] 새 창의 document가 준비된 후에 스크립트를 주입하고 실행합니다.
        ...
        <body>
            <div class="container mx-auto max-w-5xl p-4 pb-32">
                <div class="flex justify-between items-center mb-4 border-b pb-2">
                    <h1 class="text-2xl font-bold text-gray-800">토렌트 가상 폴더 뷰</h1>
    ...
    newWindow.onload = () => {
        ...
        function saveLayout() {
            const layout = [];
            for (const node of folderViewContainer.children) {
        ...
        function loadLayout() {
            ...
            savedLayout.forEach(entry => {
                if (entry.type === 'folder') {
                    ...
                    let totalSize = 0, liveItemsCount = 0;
                    ...
                    if (liveItemsCount > 0) {
                        ...
                        folderViewContainer.appendChild(details);
                    }
                } else if (entry.type === 'single' && torrentsMap.has(entry.id)) {
                    folderViewContainer.innerHTML += opener.renderTorrentItemHTML(torrentsMap.get(entry.id), true);
            ...
            if (uncategorizedItems.length > 0) {
                 folderViewContainer.innerHTML += opener.generateMonthlyGroupHTML(uncategorizedItems, true);
            }
            hasUnsavedChanges = false;
        ...
        function toggleFolderCreationMode() {
            ...
            const items = folderViewContainer.querySelectorAll('.torrent-item');
        ...
        function createVirtualFolder() {
            const selectedItems = folderViewContainer.querySelectorAll('.folder-checkbox:checked');
            if (selectedItems.length === 0) { alert("먼저 하나 이상의 항목을 선택해주세요."); return; }
            ...
            details.appendChild(folderContent);
            folderViewContainer.prepend(details);
    ...
    Object.keys(monthlyGroups).sort().reverse().forEach(key => {
        ...
        monthlyHTML += `<details class="bg-gray-100 rounded-lg mb-2">
            <summary class="p-3 cursor-pointer font-semibold text-gray-800 flex justify-between items-center hover:bg-gray-200 rounded-t-lg">
                ...
                <div class="text-sm font-normal text-gray-600">${itemsInMonth.length}개 / ${formatSizeFunc(totalSize)}</div>
            </summary>
            <div class="p-2 border-t border-gray-300">${itemsInMonth.map(t => renderFunc(t, isNewWindow)).join('')}</div>
        </details>`;
    });
    return monthlyHTML;
}
        ...
        if (items.length > 1) {
            ...
            html += `<details class="bg-gray-100 rounded-lg mb-2">
                <summary class="p-3 cursor-pointer font-semibold text-gray-800 flex justify-between items-center hover:bg-gray-200 rounded-t-lg">
                    ...
                    <div class="text-sm font-normal text-gray-600">${items.length}개 / ${formatSizeFunc(totalSize)}</div>
                </summary>
                <div class="p-2 border-t border-gray-300">${items.map(t => renderFunc(t, isNewWindow)).join('')}</div>
            </details>`;
        } else {
            singles.push(...items);
        }
